 ...
 * https://modernizr.com/download/?-mq-setclasses-shiv !*/
...
$(function(){
    $('.mobile-icon_nav,.mobile-icon_phone,.mobile-icon_search').on('click', function(){
        var $data = $(this).data('class');
        $('.temp-open').fadeOut(50);
        $($data).stop().addClass('temp-open').slideToggle(200);
    });

    $('.form-block__button').on('click', function(e){
        e.preventDefault();

        if ($(this).hasClass('form-block__button_focus')) {
            $(this).parent()
                .find('.form-block__input')
                .prop('disabled', true)
                .removeClass('form-block__input_focus');
            $(this).removeClass('form-block__button_focus').html('');
        } else {
            $(this).addClass('form-block__button_focus').html('Ok');
            $(this).parent()
                .find('.form-block__input')
                .prop('disabled', false)
                .focus()
                .addClass('form-block__input_focus');
        }

    });

    $('.form-block__input').on('focusout', function(){
        var $this =  $(this);
        setTimeout(function() {
            $this.prop('disabled', true).removeClass('form-block__input_focus');
            $this.parent()
                .find('.form-block__button')
                .html('')
                .removeClass('form-block__button_focus');
        }, 100);

    });
        

    $('.password__link').on('click', function(){
        var $arrow =$('.password__arrow');
        $arrow.hasClass('password__arrow_top') ? $arrow.removeClass('password__arrow_top').addClass('password__arrow_bottom') : $arrow.removeClass('password__arrow_bottom').addClass('password__arrow_top');
        $('.password__forms').stop().slideToggle(200);
    });
...
$(window).on('resize', function(){
    $(window).width() > 620 ? $('nav').css('display', 'block') : $('nav').css('display', 'none');
    $(window).width() > 620 ? $('.h-left, .h-right').css('display', 'table-cell') : $('.h-left, .h-right').css('display', 'none');
  });